...
async def init_db():
    ...
    # ── Schema SQL (Common to both, with slight adjustments) ──
    # Note: MySQL uses AUTO_INCREMENT, SQLite uses AUTOINCREMENT
    ...
    if db_type == "sqlite":
        ...
        if count == 0:
            ...
            await cur.execute("INSERT INTO departments (dept_name, building, head_of_dept) VALUES ('CS & IT', 'Block A', 'Dr. Hassan Ahmed')")
            await cur.execute("INSERT INTO programs (dept_id, program_name) VALUES (1, 'BS Computer Science')")
            await cur.execute("INSERT INTO students (roll_no, dept_id, program_id, name, father_name) VALUES ('CS-2026-F-001', 1, 1, 'Muhammad Ikram', 'Sher Muhammad')")
            await cur.execute("INSERT INTO fee_slips (ref_no, roll_no, amount_paid, status, challan_no) VALUES ('UOS-2026-001234', 'CS-2026-F-001', 45000, 'Verified', 'CHN-78432')")
            await cur.execute("INSERT INTO faculty (dept_id, name, designation, faculty_type) VALUES (1, 'Dr. Nadia Khan', 'Associate Professor', 'Permanent')")
            await cur.execute("INSERT INTO exam_schedules (roll_no, exam_type, exam_date, venue) VALUES ('CS-2026-F-001', 'Mid-Term', '2026-11-05', 'Main Hall')")
            await conn.commit()
...
async def get_student_info(roll_no):
    student = await fetch_one("""
        ...
        WHERE s.roll_no = ?
    """.replace("?", "%s" if DB_TYPE == "mysql" else "?"), (roll_no,))
...
async def get_fee_info(ref_no):
    return await fetch_one("""
        ...
        LEFT JOIN programs p ON s.program_id = p.program_id
        WHERE f.ref_no = ?
    """.replace("?", "%s" if DB_TYPE == "mysql" else "?"), (ref_no,))
...
async def get_db_pool():
    # Mock pool for compatibility with rag_service initialization check
    class MockPool:
